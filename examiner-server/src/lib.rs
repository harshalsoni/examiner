//! Server backend for the Examiner proctored code editor.
...
use crate::{database::Database, session::Examiner};
...
struct Document {
    ...
    session: Arc<Examiner>,
}

impl Document {
    fn new(session: Arc<Examiner>) -> Self {
        Self {
...
async fn socket_handler(id: String, ws: Ws, state: ServerState) -> Result<impl Reply, Rejection> {
    ...
    let mut entry = match state.documents.entry(id.clone()) {
        ...
        Entry::Vacant(e) => {
            let session = Arc::new(match &state.database {
                Some(db) => db.load(&id).await.map(Examiner::from).unwrap_or_default(),
                None => Examiner::default(),
            });
            if let Some(db) = &state.database {
                tokio::spawn(persister(id, Arc::clone(&session), db.clone()));
            }
            e.insert(Document::new(session))
        }
    };

    let value = entry.value_mut();
    value.last_accessed = Instant::now();
    let session = Arc::clone(&value.session);
    Ok(ws.on_upgrade(|socket| async move { session.on_connection(socket).await }))
}
...
async fn persister(id: String, session: Arc<Examiner>, db: Database) {
    ...
    while !session.killed() {
        ...
        if revision > last_revision {
            ...
            if let Err(e) = db.store(&id, &session.snapshot()).await {
